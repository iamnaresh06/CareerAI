console.log("CareerAI: Resume JS v1.0.4 - Top of file");
...
window.addItemToSection = function(slotNum) {
    console.log("CareerAI: addItemToSection called for slot", slotNum);
    ...
    console.log("CareerAI: Section type is", type);
    ...
    var htmlContent = (SECTION_TEMPLATES[type] ? SECTION_TEMPLATES[type].html : '');
    
    div.innerHTML = '<button type="button" onclick="this.parentElement.remove()" class="absolute top-2 right-2 text-muted hover:text-red-500" style="background: none; border: none; font-size: 1.2rem; cursor: pointer;"><i class="fas fa-times-circle"></i></button>' + htmlContent;
...
window.togglePresent = function(checkbox, endFieldName) {
    var parentContainer = checkbox.closest(".input-group");
    if (!parentContainer) return;
    var endInput = parentContainer.querySelector('input[name="' + endFieldName + '"]');
    if (!endInput) return;
    if (checkbox.checked) {
        endInput.value = "";
...
var SECTION_TEMPLATES = {
    experience: {
        title: "Experience",
        html: '<div class="grid grid-2 gap-4 mb-4">' +
                '<div class="input-group mb-0">' +
            ...
            '<div class="grid grid-2 gap-4 mb-4 items-end">' +
                ...
                '<div class="input-group mb-0">' +
                    ...
                    '<div class="flex gap-2 items-center">' +
                        ...
                        '<label class="flex items-center gap-1" style="white-space: nowrap; cursor: pointer; margin-bottom: 0;">' +
                            '<input type="checkbox" onchange="window.togglePresent(this, \'exp_end[]\')" style="width: auto;">' +
                            '<span>Present</span>' +
                        '</label>' +
                    '</div>' +
                '</div>' +
